    ...
    <div class="min-h-screen bg-gradient-to-b from-base-200/30 to-base-100 pb-20 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div class="fixed top-6 right-6 z-[100] flex flex-col gap-4 w-72 md:w-96 pointer-events-none">
        <%= if msg = Phoenix.Flash.get(@flash, :error) do %>
          <div class="flex items-start gap-4 bg-error/90 backdrop-blur-md text-error-content p-5 rounded-2xl shadow-2xl animate-in slide-in-from-right-8 duration-500 pointer-events-auto">
            <div class="p-1.5 bg-error-content/20 rounded-xl shrink-0"><.icon name="hero-exclamation-triangle-solid" class="size-6" /></div>
            <p class="font-black text-[11px] uppercase tracking-widest leading-relaxed mt-1"><%= msg %></p>
          </div>
        <% end %>
        <%= if msg = Phoenix.Flash.get(@flash, :info) do %>
          <div class="flex items-start gap-4 bg-success/90 backdrop-blur-md text-success-content p-5 rounded-2xl shadow-2xl animate-in slide-in-from-right-8 duration-500 pointer-events-auto">
            <div class="p-1.5 bg-success-content/20 rounded-xl shrink-0"><.icon name="hero-check-circle-solid" class="size-6" /></div>
            <p class="font-black text-[11px] uppercase tracking-widest leading-relaxed mt-1"><%= msg %></p>
          </div>
        <% end %>
      </div>
      ...
      <%!-- NAV --%>
      <nav class="bg-base-100/80 backdrop-blur-2xl border-b border-base-200/60 sticky top-0 z-40 shadow-sm mb-8 md:mb-12">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <.link navigate={~p"/cliente/sorteos"} class="p-3 bg-base-200/50 hover:bg-primary/10 hover:text-primary rounded-2xl transition-all group flex items-center gap-3">
          ...
          <div class="flex items-center gap-4 md:gap-6">
            <button phx-click="abrir_historial" class="flex flex-col items-end px-4 border-r border-base-300/50 hover:opacity-70 transition-opacity text-right">
              ...
              <p class="text-xl md:text-2xl font-black leading-none italic text-success">$<%= @balance.saldo %></p>
            </button>
            <button phx-click="abrir_modal" class="btn btn-primary h-12 px-6 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20 hover:-translate-y-1 transition-all gap-2">
            ...
            <ul class="flex flex-col gap-2">
              <li>
                <button phx-click="set_seccion" phx-value-sec="info"
                  class={["w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all duration-300",
                    @seccion_activa == "info" && "bg-primary text-primary-content shadow-lg shadow-primary/30",
                    @seccion_activa != "info" && "text-base-content/50 hover:bg-base-200/80 hover:text-base-content"]}>
              ...
              <li>
                <button phx-click="set_seccion" phx-value-sec="seguridad"
                  class={["w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all duration-300",
                    @seccion_activa == "seguridad" && "bg-base-content text-base-100 shadow-lg shadow-base-content/20",
                    @seccion_activa != "seguridad" && "text-base-content/50 hover:bg-base-200/80 hover:text-base-content"]}>
                    ...
                    <%= if @editando_campo == "nombre" do %>
                      <form phx-submit="guardar_cambios" class="flex flex-col sm:flex-row gap-3">
                        ...
                        <div class="flex gap-2">
                          <button class="btn btn-primary rounded-xl px-6"><.icon name="hero-check" class="size-5" /></button>
            ...
            <div class="relative bg-base-100/90 backdrop-blur-3xl rounded-[3rem] shadow-2xl border border-base-200/60 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div class="relative p-8 md:p-12 border-b border-base-200/60 overflow-hidden">
                ...
                <div class="relative z-10">
                  ...
                  <p class="text-[11px] font-black uppercase tracking-widest text-base-content/40 mt-3">Tickets comprados y su estado</p>
                </div>
              </div>
                ...
                <% else %>
                  <%= for ticket <- @tickets_usuario do %>
                    <div class={[
                      ...
                      ticket.estado != "vendido" && "bg-success/5 border-success/20 opacity-50"
                    ]}>
                      <div class="flex items-center gap-4">
                        <div class="size-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner bg-primary/10 text-primary">
                          <%= ticket.numero %>
                        </div>
                        <div>
                      ...
                      <%= if ticket.estado == "vendido" and ticket.sorteo.estado == "activo" do %>
                        <button phx-click="devolver_ticket" phx-value-ticket_id={ticket.id}
                          data-confirm={"¿Devolver el ticket ##{ticket.numero}? Se te reembolsará $#{ticket.sorteo.precio_ticket}."}
                          ...
                          <.icon name="hero-arrow-uturn-left-solid" class="size-4" /> Devolver
                        </button>
                      <% end %>
                      <%= if ticket.sorteo.estado == "finalizado" do %>
                        <% es_ganador = ticket.numero in (ticket.sorteo.numeros_ganadores || []) %>
                        <div class={[
                          "px-4 py-2 rounded-2xl font-black text-[10px] uppercase tracking-widest shrink-0 border",
                          es_ganador && "bg-warning/10 text-warning border-warning/20",
                          not es_ganador && "bg-base-300/30 text-base-content/30 border-base-300/20"
                        ]}>
            ...
            <div class="relative bg-base-100/90 backdrop-blur-3xl rounded-[3rem] shadow-2xl border border-base-200/60 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div class="relative p-8 md:p-12 border-b border-base-200/60 overflow-hidden">
                ...
                <div class="relative z-10">
                  ...
                  <p class="text-[11px] font-black uppercase tracking-widest text-base-content/40 mt-3">Resumen de cada sorteo en que participaste</p>
                </div>
              </div>
                    ...
                    <div class={[
                      ...
                      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <%= if item.sorteo.estado == "finalizado" do %>
                          ...
                          %>
                          <div class={[
                            "p-3 rounded-2xl border col-span-2",
          ...
          <div class="relative bg-base-100/95 backdrop-blur-3xl border border-base-200/60 rounded-[3rem] p-6 md:p-10 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div class="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-[2rem] p-6 mb-6 flex items-center justify-between">
              <div>
                ...
                <p class="text-4xl font-black italic text-primary leading-none mt-1">$<%= @balance.saldo %></p>
              </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              <div class="bg-base-200/50 p-4 rounded-2xl border border-base-300/30 flex flex-col gap-1">
                ...
                <span class="text-2xl font-black italic"><%= @balance.tickets %></span>
              </div>
              <div class="bg-base-200/50 p-4 rounded-2xl border border-base-300/30 flex flex-col gap-1">
                <span class="text-[9px] font-black uppercase tracking-widest text-base-content/40 flex items-center gap-1">
                  <.icon name="hero-sparkles-solid" class="size-3" /> Sorteos
                </span>
                <span class="text-2xl font-black italic"><%= @balance.sorteos %></span>
              </div>
            </div>
            ...
            <div class={[
              ...
              if(@balance.es_ganancia, do: "bg-success/10 border-success/20", else: "bg-error/10 border-error/20")
            ]}>
              <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <%= for amount <- [10000, 20000, 50000, 100000] do %>
                    <button type="button" phx-click="seleccionar_monto" phx-value-monto={amount}
                      class={["h-14 rounded-2xl font-black text-sm md:text-base italic transition-all border",
                        @modo_monto == "rapido" && @monto_seleccionado == amount && "bg-primary text-primary-content border-primary shadow-lg shadow-primary/30 scale-[1.02]",
                        !(@modo_monto == "rapido" && @monto_seleccionado == amount) && "bg-base-200/50 border-base-300/50 text-base-content/70 hover:bg-base-200"]}>
                      $<%= amount %>
                    </button>
                  <% end %>
                  <button type="button" phx-click="activar_manual"
                    class={["col-span-2 md:col-span-1 h-14 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all border flex items-center justify-center gap-2",
                      @modo_monto == "manual" && "bg-secondary text-secondary-content border-secondary shadow-lg shadow-secondary/30 scale-[1.02]",
                      @modo_monto != "manual" && "bg-base-200/50 border-base-300/50 text-base-content/70 hover:bg-base-200"]}>
                ...
                <div class="grid grid-cols-3 gap-3">
                  <%= for {m, icon, label} <- [{"pse", "hero-building-library-solid", "PSE"}, {"tarjeta", "hero-credit-card-solid", "Tarjeta"}, {"efectivo", "hero-banknotes-solid", "Efectivo"}] do %>
                    <button type="button" phx-click="seleccionar_metodo" phx-value-metodo={m}
                      class={["flex flex-col items-center justify-center gap-2 h-24 rounded-[1.5rem] border transition-all",
                        @metodo_pago == m && "bg-base-content text-base-100 border-base-content shadow-lg shadow-base-content/20 scale-[1.02]",
                        @metodo_pago != m && "bg-base-200/50 border-base-300/50 text-base-content/60 hover:bg-base-200"]}>
